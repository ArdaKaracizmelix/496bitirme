        ...
        if serializer.is_valid():
            try:
                with transaction.atomic():
                    ...
                    verification_url = build_email_verification_url(request, token)
                ...
                return Response(
                    {"detail": "Unable to send verification email. Please try registering again."},
...
class InterestAvailableView(APIView):
    ...
    def get(self, request):
        interests = _interest_catalog()
        live_table_a_count = len(_fetch_google_table_a_types(include_static=False))
        return Response(
            {
                "interests": interests,
                "source": "google_table_a" if live_table_a_count > 0 else "static_snapshot",
                "table_a_count": live_table_a_count,
                "snapshot_table_a_count": len(GOOGLE_TABLE_A_STATIC_TYPES),
            },
            status=status.HTTP_200_OK,
        )
...
class InterestSourceHealthView(APIView):
    ...
    def get(self, request):
        effective_types = _fetch_google_table_a_types(include_static=True)
        live_types = _fetch_google_table_a_types(include_static=False)
        diagnostics = {
            "doc_url": GOOGLE_TABLE_A_DOC_URL,
            "live_fetch_ok": len(live_types) > 0,
            "live_table_a_count": len(live_types),
            "effective_table_a_count": len(effective_types),
            "snapshot_table_a_count": len(GOOGLE_TABLE_A_STATIC_TYPES),
            "mode": "hybrid_live_plus_snapshot" if len(live_types) > 0 else "snapshot_only",
        }
        ...
        try:
            ...
            parsed_types = _extract_table_a_types_from_html(html)

            diagnostics["live_table_a_count"] = len(parsed_types)
            diagnostics["table_a_markers_found"] = {
                "start_found": start != -1,
                "end_found": end != -1,
            }
...
class InterestSubmitView(APIView):
    ...
    def post(self, request):
        ...
        catalog = _interest_catalog()
        category_by_id = {item["id"]: item["name"] for item in catalog}
        normalized_name_map = {
            str(item["name"]).strip().upper(): item["name"] for item in catalog
        }
        ...
        for item in interest_ids:
            if isinstance(item, int) and item in category_by_id:
                ...
                if normalized in normalized_name_map:
                    selected_categories.append(normalized_name_map[normalized])

        # Expand top-level Google groups (e.g. CULTURE) into concrete place types.
        expanded_categories = []
        for category in selected_categories:
            group_children = GOOGLE_INTEREST_GROUPS.get(category)
            if group_children:
                expanded_categories.extend(group_children)
            else:
                expanded_categories.append(category)

        expanded_categories = list(dict.fromkeys(expanded_categories))
        ...
        profile, _ = UserProfile.objects.get_or_create(user=request.user)
        profile.preferences_vector = {category: 1.0 for category in expanded_categories}
            ...
            interests.append(
                {
                    "id": matched_id,
                    "name": category,
                    "title": _format_interest_label(category),
                }
            )
